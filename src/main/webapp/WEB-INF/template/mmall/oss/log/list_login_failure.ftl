...
<head>
  ...
  <title>log_login_success-list</title>
<#include "../head.ftl"/>
  <script type="text/javascript">
    ...
    function optDelete() {
      ...
      var f = getTableForm();
      f.action = "o_remove_login_failure.jhtml";
      f.submit();
    }
  </script>
</head>
...
<div class="box-positon">
  <div class="rpos">当前位置: 登录失败日志 - 列表</div>
  <form class="ropt" action="o_remove_login_failure_batch.jhtml" method="post">
    <span>批量删除</span>
      <select name="days">
...
<div class="body-box">
  <form action="v_list_login_failure.jhtml" method="post" style="padding-top:5px;">
  ...
  <@t.table listAction="v_list_login_failure.jhtml" value=pagination;ossLog,i,has_next><#rt/>
  ...
  <@t.column title="日志内容">${ossLog.content}</@t.column><#t/>
  <@t.column title="操作选项" align="center">
    <a href="o_remove_login_failure.jhtml?ids=${ossLog.logId}&pageNo=${pageNo!}&queryUsername=${queryUsername!}&queryTitle=${queryTitle!}" onclick="if(!confirm('您确定删除吗？')) {return false;}"
       class="pn-opt">删除</a><#t/>
  </@t.column><#t/>
  </@t.table>
